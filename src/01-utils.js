...
// === COPY TO CLIPBOARD ===
var _cpvSvg = '<span class="cpv-ico"><svg class="ci-clip" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg><svg class="ci-check" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#16a34a" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span>';
function copyVal(text, el) {
  if (!text) return;
  var clean = text.replace(/\s+/g, ' ').trim();
  navigator.clipboard.writeText(clean).then(function() {
    if (el) {
      el.classList.add('cpv-ok');
      setTimeout(function() { el.classList.remove('cpv-ok'); }, 1200);
    }
  }).catch(function() {
    ...
    if (el) { el.classList.add('cpv-ok'); setTimeout(function(){ el.classList.remove('cpv-ok'); }, 1200); }
  });
}
function _cpv(text) {
  var safe = String(text).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/'/g,'&#39;').replace(/</g,'&lt;');
  return '<span class="cpv" data-cpv="' + safe + '" onclick="event.stopPropagation();copyVal(this.dataset.cpv,this)">' + esc(text) + _cpvSvg + '</span>';
}
function _cpvRaw(text, html) {
  var safe = String(text).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/'/g,'&#39;').replace(/</g,'&lt;');
  return '<span class="cpv" data-cpv="' + safe + '" onclick="event.stopPropagation();copyVal(this.dataset.cpv,this)">' + html + _cpvSvg + '</span>';
}
